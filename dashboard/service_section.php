...
<?php
	...
	$route = "pages/services/sections/main-section";
	/* Page Route Ends Here*/
	$errors = []; $status = false; $msg = ""; 
	$editMode = false;
	$formUrl = generate_route($route);
	$formTitle = "Add Service Section Content";
	$advert = '';
	...
	if(is_post_request()){
		...
		if(isset($_GET["mode"]) && isset($id)){
			...
			$formTitle = "Edit Service section";
			switch ($_GET["mode"]) {
				case 'edit':
					...
					if(!$valResult){
						...
						$data = find_data('page_datas',['id'],' WHERE page_datas.title = "service-section-content" AND page_datas.id ='.merge_and_escape([$id],$db).' LIMIT 1', false);
						if($data){
							// File Exist
			...
			if(!$valResult){
				$data["content"] =  array_to_json($section,'csrf_token');
				$data["title"] = "service-section-content";
		...
		if(isset($_GET["mode"]) && isset($id)){
			...
			switch ($_GET["mode"]) {
				case 'edit':
					...
					$data  = find_data('page_datas',['page_datas.id','content']," WHERE page_datas.title='service-section-content' AND page_datas.id = ".merge_and_escape([$id],$db)." LIMIT 1",false);
					if($data){
			...
			$data = find_data('page_datas',['content','page_datas.id'], ' WHERE page_datas.title ="service-section-content" LIMIT 1', false);
			if($data){
