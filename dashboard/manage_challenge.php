...
<?php
	...
	if(is_post_request()){
		...
		if(isset($_GET["mode"]) && isset($id)){
			...
			$challenges  = find_data('page_datas',['page_datas.id','content','path','page_datas.date_created'],'INNER JOIN files ON page_datas.file_id = files.id',"WHERE page_datas.title='about-challenge-item' ORDER BY id desc",false);
		}
