...
<?php
	...
	$route = "pages/about-us/sections/core-value";
	/* Page Route Ends Here*/
	$errors = []; $status = false; $msg = ""; 
	$editMode = false;
	$formUrl = generate_route($route);
	$formTitle = "Add core values section Content";
	$advert = '';
	...
	if(is_post_request()){
		...
		if(isset($_GET["mode"]) && isset($id)){
			...
			$formTitle = "Edit core values section";
			switch ($_GET["mode"]) {
				case 'edit':
					...
					if(!$valResult){
						...
						$data = find_data('page_datas',['id'],' WHERE page_datas.title = "core-value-about" AND page_datas.id ='.merge_and_escape([$id],$db).' LIMIT 1', false);
						if($data){
							// File Exist
		...
		if(isset($_GET["mode"]) && isset($id)){
			...
			switch ($_GET["mode"]) {
				case 'edit':
					...
					$data  = find_data('page_datas',['page_datas.id','content']," WHERE page_datas.title='core-value-about' AND page_datas.id = ".merge_and_escape([$id],$db)." LIMIT 1",false);
					if($data){
						$section = json_to_array($data["content"]);
						$formUrl = generate_route($route, "edit", $id);
						$formTitle = "Edit core value section";
			...
			$data = find_data('page_datas',['content','page_datas.id'], ' WHERE page_datas.title ="core-value-about" LIMIT 1', false);
			if($data){
						...
						<div class="row d-flex align-items-center">
							<div class="col-md-6">
								<div class="page-breadcrumb">
									<h1>Manage core value section</h1>
								</div>
							</div>
