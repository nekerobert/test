...
<?php
	...
	$route = "pages/services/sections/footer-banner";
	...
	if(is_post_request()){
		...
		if(isset($_GET["mode"]) && isset($id)){
			...
			$formTitle = "Edit Service footer banner";
			switch ($_GET["mode"]) {
				case 'edit':
					...
					if(!$valResult){
						...
						$data = find_data('page_datas',['id'],' WHERE page_datas.title = "service-footer-banner" AND page_datas.id ='.merge_and_escape([$id],$db).' LIMIT 1', false);
						if($data){
							// File Exist
							...
							$msg = "Service footer banner updated successfully";
						}else{
			...
			if(!$valResult){
				$data["content"] =  array_to_json($banner,'csrf_token');
				$data["title"] = "service-footer-banner";
		...
		if(isset($_GET["mode"]) && isset($id)){
			...
			switch ($_GET["mode"]) {
				case 'edit':
					...
					$data  = find_data('page_datas',['page_datas.id','content']," WHERE page_datas.title='service-footer-banner' AND page_datas.id = ".merge_and_escape([$id],$db)." LIMIT 1",false);
					if($data){
			...
			$data = find_data('page_datas',['content','page_datas.id'], ' WHERE page_datas.title ="service-footer-banner" LIMIT 1', false);
			if($data){
				$banner = sanitize_html(json_to_array($data["content"]));
				$banner["id"] = h(u($data["id"]));
				$formUrl = generate_route($route, "edit", $banner["id"]);;
				$formTitle = "Edit Services footer banner";
			}
						...
						<div class="row d-flex align-items-center">
							<div class="col-md-6">
								<div class="page-breadcrumb">
									<h1>Manage Service Footer Banner</h1>
								</div>
							</div>
