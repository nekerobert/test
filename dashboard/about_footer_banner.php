...
<?php
	...
	$route = "pages/about-us/sections/footer-banner";
	...
	if(is_post_request()){
		...
		if(isset($_GET["mode"]) && isset($id)){
			...
			$formTitle = "Edit About Us footer banner";
			switch ($_GET["mode"]) {
				case 'edit':
					...
					if(!$valResult){
						...
						$data = find_data('page_datas',['id'],' WHERE page_datas.title = "about-footer-banner" AND page_datas.id ='.merge_and_escape([$id],$db).' LIMIT 1', false);
						if($data){
							// File Exist
							...
							$msg = "About Us footer banner updated successfully";
						}else{
			...
			if(!$valResult){
				$data["content"] =  array_to_json($banner,'csrf_token');
				$data["title"] = "about-footer-banner";
		...
		if(isset($_GET["mode"]) && isset($id)){
			...
			switch ($_GET["mode"]) {
				case 'edit':
					...
					$data  = find_data('page_datas',['page_datas.id','content']," WHERE page_datas.title='about-footer-banner' AND page_datas.id = ".merge_and_escape([$id],$db)." LIMIT 1",false);
					if($data){
			...
			$data = find_data('page_datas',['content','page_datas.id'], ' WHERE page_datas.title ="about-footer-banner" LIMIT 1', false);
			if($data){
						...
						<div class="row d-flex align-items-center">
							<div class="col-md-6">
								<div class="page-breadcrumb">
									<h1>Manage About-us Footer Banner</h1>
								</div>
							</div>
