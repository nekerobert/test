	...
	if(is_post_request()){
		...
		if(isset($_GET["mode"]) && isset($id)){
			...
			if(!$valResult){
				...
				$faqs  = find_data('page_datas',["id","title","content","date_created"],null, 'WHERE title = "main_faq"');
			}else{
				// There is errors
				$errors = $valResult;
				// Retrieve record to be displayed on the table again
			...
			switch ($_GET["mode"]) {
				case 'edit':
					...
					if($data){
                        ...
                        $faqs  = find_data('page_datas',["id","title","content","date_created"],null, 'WHERE title = "main_faq"', false);
					}else{
