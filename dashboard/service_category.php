   ...
   /* Page Route Ends Here*/
	...
	if(is_post_request()){
			...
			switch ($_GET["mode"]) {
				case 'edit':
					...
					if(!$valResult){
						...
						// Errors Occured
						$errors = $valResult;
						$categories = find_data('categories',['id','cat_title','type','date_created'], null, ' WHERE type="services" ORDER BY id desc');		
					}
		...
		if(isset($_GET["mode"]) && isset($id)){
			...
			switch ($_GET["mode"]) {
				case 'edit':
					$category = find_data_by_id('categories',['cat_title','cat_desc'],$id);
					if($category){
									...
									<div class="card card-shadow mb-4">
										<div class="card-header bg-info ">
											<div class="card-title text-white">
												Manage Service Categories (<i class="fa fa-book"></i>) 
											</div>
										</div>
								...
								<div class="card-body card-responsive">
									<form id= "form" action="<?php echo $formUrl; ?>" method="post">
                                        ...
                                        <div class="form-group">
												...
												<textarea id="category-desc" class="form-control" name="cat_desc" rows="8" cols="4"><?php echo $category["cat_desc"] ?? ""; ?></textarea>
											</div>
