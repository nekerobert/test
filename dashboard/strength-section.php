...
<?php
	...
	$route = "pages/home/sections/key-strength";
	/* Page Route Ends Here*/
	$errors = []; $status = false; $msg = ""; 
	$editMode = false;
	$formUrl = generate_route($route);
	$formTitle = "Add Key Strength Content";
	$advert = '';
	...
	if(is_post_request()){
        ...
        // Also Triming of form data is done here
		...
		if(isset($_GET["mode"]) && isset($id)){
			...
			$formTitle = "Edit Key strength Section";
			switch ($_GET["mode"]) {
				case 'edit':
					...
					if(!$valResult){
						...
						$data = find_data('page_datas',['id'],' WHERE page_datas.title = "home-key-strength" AND page_datas.id ='.merge_and_escape([$id],$db).' LIMIT 1', false);
						if($data){
							// File Exist
							...
							$msg = "Key Strength Section was updated successfully";
						}else{
		...
		if(isset($_GET["mode"]) && isset($id)){
			...
			switch ($_GET["mode"]) {
				case 'edit':
					...
					$data  = find_data('page_datas',['page_datas.id','content']," WHERE page_datas.title='home-key-strength' AND page_datas.id = ".merge_and_escape([$id],$db)." LIMIT 1",false);
					if($data){
			...
			$data = find_data('page_datas',['content','page_datas.id'], ' WHERE page_datas.title ="home-key-strength" LIMIT 1', false);
			if($data){
						...
						<div class="row d-flex align-items-center">
							<div class="col-md-6">
								<div class="page-breadcrumb">
									<h1>Manage Key Strength Section</h1>
								</div>
							</div>
								...
								<div class="card-body">
									<form id="form" method="post" class=" right-text-label-form feedback-icon-form" action="<?php echo $formUrl; ?>">
											<div class="form-group">
												<label class="control-label" for="section-title">Key Strength Section Title</label>
											...
											<div class="form-group">
												<label class="control-label" for="section-desc">Section Description</label>
												<textarea id="section-desc" class="form-control" name="section_desc" rows="10">
												<?php echo $section["section_desc"] ?? ""; ?>
												</textarea>
											</div>
